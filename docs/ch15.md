...
> 即使您确实有自律性可以回去写评论（也不要自欺欺人：您可能没有），但是评论也不是很好。到此时为止，您已经在心理上退房了。在您看来，这段代码已经完成；您渴望继续进行下一个项目。您知道写评论是正确的事情，但这并不有趣。您只想尽快解决它。因此，您可以快速遍历代码，仅添加足够多的注释以使其看起来受人尊敬。到目前为止，自设计代码以来已有一段时间，因此您对设计过程的记忆正变得模糊。您在编写注释时会查看代码，因此注释会重复代码。即使您尝试重新构造代码中并不明显的设计思想，也会有一些您不记得的事情。从而，
