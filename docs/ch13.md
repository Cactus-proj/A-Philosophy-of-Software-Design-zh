...
> 不幸的是，许多注释并不是特别有用。最常见的原因是注释重复了代码：可以轻松地从注释旁边的代码中推断出注释中的所有信息（最常见的就是重复了代码的注释：对可以从代码中轻松推断出来的信息进行注释）。这是最近研究论文中出现的代码示例：
...
> 编写注释后，请问自己以下问题：从未看过代码的人能否仅通过查看注释旁边的代码来编写评论（从没有看过代码的人是否可以仅仅看一下代码就可以理解其具体功能（仅仅看一眼代码是否就可以写相关注释了））？如果答案是肯定的（如上述示例所示），则注释不会使代码更易于理解。像这样的评论是为什么有些人认为评论毫无价值的原因。
...
> 如果注释旁边的代码中的注释信息已经很明显，则注释无济于事(如果注释中的信息已经通过代码很明显的表达出来了，那这个注释就没有太大用处；另外一个翻译：如果代码基本都够将信息完整展示出来，那这个注释就不会有太多用处)。这样的一个例子是，当注释使用与所描述事物名称相同的单词时。
...
> 同时，注释中缺少一些重要信息（而且上面的示例中还遗失了一些重要信息）：例如，什么是“标准化资源名称”（什么是“normalized resource name”），以及 getNormalizedResourceNames 返回的数组的元素是什么？“downcast”是什么意思？填充的单位是什么，填充是在每行的一侧还是在两者的两侧？在注释中描述这些内容将很有帮助。
...
> 该评论太底层和太详细。一方面，它部分重复了代码（它重复了部分代码）：“如果有 LOADING readRPC”仅重复测试 `readRpc[i].status == LOADING`（“if there is a LOADING readRPC”就是重复了`readRpc[i].status == LOADING`这一判断）。另一方面，注释不能解释此代码的总体目的，也不能解释其如何适合包含此代码的方法。如此一来注释不能帮助读者理解代码。
