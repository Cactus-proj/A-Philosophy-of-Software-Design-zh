...
> 不幸的是，在许多情况下，放置跨模块文档并没有明显的中心位置。RAMCloud 存储系统的一个示例是处理僵尸服务器的代码，这些僵尸服务器是系统认为已崩溃但实际上仍在运行的服务器。中立的僵尸服务器需要几个不同模块中的代码，并且这些代码段彼此依赖。没有哪段代码是放置文 ​​ 档的明显中心位置。一种可能是在依赖它的每个位置复制文档的各个部分。但是，这很尴尬，并且随着系统的发展，很难保持此类文档为最新。另外，也可以将文档放在需要的地方之一，
