...
> 考虑在第六章和第七章中讨论过的为 GUI 文本编辑器管理文件文本的类。该类提供了一些方法，可将文件从磁盘读取到内存中，查询和修改文件的内存中副本，并将修改后的版本写回到磁盘。当学生不得不实施该课程时，他们中的许多人选择了一个面向行的界面，该界面具有读取，插入和删除整行文本的方法。这为该类提供了一个简单的实现，但为更高级别的软件带来了复杂性。在用户界面级别，操作很少涉及整行。例如，击键会导致将单个字符插入到现有行中。复制或删除所选内容可以修改几行不同的内容。通过面向行的文本界面，
