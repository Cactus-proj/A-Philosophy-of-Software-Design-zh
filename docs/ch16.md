...
> 如果没有“明显”的地方可以将特定文档放置在开发人员可以找到的地方，请按照第 13.7 节中的描述创建一个 designNotes 文件。或者，选择最好的可用位置，然后将文档放在此处。另外，在其他引用中心位置的地方添加简短注释：“请参见 xyz 中的注释，以获取以下代码的解释。” 如果由于主注释被移动或删除而导致引用过时，则这种矛盾是不言而喻的，因为开发人员无法在指定的位置找到该注释。他们可以使用修订控制历史记录来查找注释发生了什么，然后更新引用。相反，如果文档重复并且某些副本没有更新，
...
> 如果信息已经在程序外部的某个地方进行了记录，请不要重复该程序内部的记录；只需参考外部文档即可。例如，如果您编写实现 HTTP 协议的类，则无需在代码内部描述 HTTP 协议。Web 上已经有许多有关此文档的资源。只需使用以下来源之一的 URL 在代码中添加简短注释即可。另一个示例是用户手册中已记录的功能。假设您正在编写一个实现命令集合的程序，其中一个方法负责实现每个命令。如果有描述这些命令的用户手册，则无需在代码中重复此信息。代替，
