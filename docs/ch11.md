...
设计两次不需要花费很多额外的时间。对于较小的模块比如类，您可能仅需一两个小时来斟酌不同的方案。与您将花费数天或数周时间来实现该类相比，这是很少的时间。初步的设计尝试可能会产生明显更好的设计，带来的好处远超你在设计两次上时间花销。对于较大的模块，您将花费更多的时间进行初始设计探索，但是实现也将花费更长的时间，并且更好的设计所带来的好处也会更高。
