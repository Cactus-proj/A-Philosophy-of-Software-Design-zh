...
> 未知未知数：复杂性的第三个症状是，不清楚必须修改哪些代码才能完成任务，或者开发人员必须成功地执行任务才能获得哪些信息。图 2.1（c）说明了此问题。该网站使用中央变量来确定横幅背景色，因此看起来很容易更改。但是，一些网页使用深色的背景色进行强调，并且该深色是在各个页面中明确指定的。如果背景颜色改变，则强调颜色必须改变以匹配。不幸的是，开发人员不太可能意识到这一点，因此他们可以在不更新强调颜色的情况下更改中央 bannerBg 变量。即使开发人员意识到了问题所在，也不清楚哪个页面使用强调颜色，
...
> 在复杂性的三种表现形式中，未知的未知数最严重。一个未知的未知意味着您需要了解一些东西，但是您无法找到它是什么，甚至是否有问题。进行更改后，直到出现错误，您才能找到有关它的信息。变更放大很烦人，但是只要清楚需要修改哪些代码，变更完成后系统就可以工作。同样，较高的认知负担会增加更改的成本，但如果清楚读取哪些信息，则更改仍然很可能是正确的。对于未知的未知数，尚不清楚该怎么办或提议的解决方案是否将起作用。唯一可以确定的方法是读取系统中的每一行代码，这对于任何规模的系统都是不可能的。即使这可能还不够，
...
> 造成复杂性的第二个原因是模糊性。当重要信息不明显时，就会发生混淆。一个简单的例子是变量名，它是如此通用，以至于它没有携带太多有用的信息（例如时间）。或者，变量的文档可能未指定其单位，因此找出的唯一方法是扫描代码以查找使用该变量的位置。模糊性通常与依赖关系相关，在这种情况下，并不明显存在依赖关系。例如，如果将新的错误状态添加到系统，则可能有必要向包含每个状态的字符串消息的表中添加条目，但是消息表的存在对于程序员查看状态可能并不明显宣言。不一致也是导致晦涩的主要原因：如果将相同的变量名用于两个不同的目的，
